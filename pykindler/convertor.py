...
def process_and_convert_books(file_list):
    for filename in file_list:
        absolute_file_path = path.join(CONST['downloads_dir'], filename)

        # Move mobi files as is
        if filename.endswith('.mobi'):
            rename(absolute_file_path, path.join(CONST['convert_dir'], filename))
            continue

        # Don't convert files which are too big
        file_size_in_mb = path.getsize(absolute_file_path)/1e6
        if file_size_in_mb > CONST['conversion_threshold_in_mb']:
            continue

        if True in set(filename.endswith(extension) for extension in CONST['extension_list']):
            ...
            try:
                ...
                temp_metadata_storage = open(CONST['temp_metadata_file'], "w")
                run(CONST['metadata_command'](cleaned_file_name), stdout=temp_metadata_storage, check=True)

                #Move conversion to Converted and book to Processed
                print(f"Beginning conversion: {filename}")
                check_output(['ebook-convert',
                              absolute_file_path,
                              path.join(CONST['convert_dir'], filename[:filename.rfind('.')]+'.mobi')
                             ])
                rename(absolute_file_path, path.join(CONST['processed_dir'],filename))
    ...
    # Cleanup
    if path.exists(CONST['temp_metadata_file']):
        remove(CONST['temp_metadata_file'])